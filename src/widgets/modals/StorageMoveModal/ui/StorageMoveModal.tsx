...
const StorageMoveModal = (props: PropsType) => {
  ...
  const storages = useSelector(
    getFilteredStoragesList((storage) => storage.id !== currentStorage.id),
  );
  const [isLoading, setIsLoading] = useState(false);
  const { displayNotification } = useStorageResponseNotification();

  const handleSubmit = useCallback(
    (values: FormStateType, formikHelpers: FormikHelpers<FormStateType>) => {
      setIsLoading(true);
      ...
      bodyElement={
        <>
          <FormControl mb="20px" isInvalid={!!formErrors.destinationStorageId}>
            ...
            <FormErrorMessage>
              {formErrors.destinationStorageId}
            </FormErrorMessage>
          </FormControl>
          <CountInput
