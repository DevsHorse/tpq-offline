...
class StorageDB {
	...
	updateStorage(options: UpdateStorageOptions) {
		return new Promise((resolve, reject) => {
			...
			case UpdateStorageType.ADD: {
				const storageIndex = storages.findIndex(storage => storage.id === data.storageId);
			...
			case UpdateStorageType.MOVE: {
				const storageIndex1 = storages.findIndex(storage => storage.id === data.sourceStorageId);
				const storageIndex2 = storages.findIndex(storage => storage.id === data.destinationStorageId);
	...
	removeAction(id: number) {
		...
		if (items) {
			...
			actions = actions.filter((action: OfflineHistoryAction) => action.id !== id);
			localStorage.setItem(this.lsStoragesActionsKey, JSON.stringify(actions));
		}
	}
