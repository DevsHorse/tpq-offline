...
const storagesPageSlice = createSlice({
	...
	reducers: {
		updateStorage(state: StoragesPageSchema, action: PayloadAction<IStorage>) {
			const index = state.storages.findIndex(storage => storage.id === action.payload.id);
	...
	extraReducers: (builder) => {
		builder.addCase(getStorages.pending, (state: StoragesPageSchema) => {
			state.isLoading = true;
		}).addCase(getStorages.fulfilled, (state: StoragesPageSchema, action: PayloadAction<IStorage[]>) => {
