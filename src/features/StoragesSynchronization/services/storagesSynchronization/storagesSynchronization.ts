...
import {OfflineHistoryAction, UpdateStorageType, StorageApi} from '../../../../entities/Storage';
	...
	async (data, thunkApi) => {
		...
		try {
			if (isOnline()) {
				...
				const actions: OfflineHistoryAction[] = await storageApi.offlineApi.getActions();
				...
				await actionsRequests.reduce(
					(sequence: Promise<any>, promise) => sequence.then(promise),
					Promise.resolve()
				).then((result: SynchronizationInformation[] | void) => {
					if (result) {
						dispatch(storagesSynchronizationActions.setSynchronizationInformation(result));
					}
				});
			}
		} catch (e) {
