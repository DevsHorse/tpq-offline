...
const storagesSynchronizationSlice = createSlice({
	...
	reducers: {
		...
		setSynchronizationInformation(state: StoragesSynchronizationSchema, action: PayloadAction<SynchronizationInformation[]>) {
			state.synthInformation = action.payload;
		}
	},
	extraReducers: (builder) => {
		builder.addCase(storagesSynchronization.pending, (state: StoragesSynchronizationSchema) => {
			state.isLoading = true;
		}).addCase(storagesSynchronization.fulfilled, (state: StoragesSynchronizationSchema) => {
			state.isLoading = false;
		}).addCase(storagesSynchronization.rejected, (state: StoragesSynchronizationSchema) => {
			state.isLoading = false;
		});
	}
});

export const { reducer: storagesSynchronizationReducer, actions: storagesSynchronizationActions } =
